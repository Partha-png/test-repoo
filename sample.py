...
def is_number(num: object) -> bool:
    return isinstance(num, (int, float)) or isinstance(num, str) and num != ''
...
def process_numbers(numbers: Iterable[Union[int, float, str]]) -> tuple[float, list[float]]:
    ...
    total = sum(to_float(num) for num in numbers)
    return total, list(to_float(num) for num in numbers)
