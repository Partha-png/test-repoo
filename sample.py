...
class Calculator:
    def process_numbers(self, numbers: Iterable[Union[int, float, str]]) -> Tuple[float, list[float]]:
        ...
        non_negative_sum = sum(num for num in numbers if isinstance(num, (int, float)) and num >= 0)
        return non_negative_sum, [num for num in numbers if isinstance(num, (int, float))]
