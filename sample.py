...
class Calculator:
    ...
    def validate_numbers(self, numbers: Iterable[Number]) -> Tuple[float, list[float]]:
        return self._validate_numbers(numbers)

    def _validate_numbers(self, numbers: Iterable[Number]) -> Tuple[float, list[float]]:
        processed = self.process_numbers(numbers)
        return sum(num.to_float() for num in numbers if isinstance(num, (int, float)) and num.to_float() >= 0), processed
