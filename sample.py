...
class Number(Protocol):
    def to_float(self) -> float:
        """Abstract method to convert number to float."""
        pass

@dataclass
class Calculator:
    def process_numbers(self, numbers: Iterable[Number]) -> list[float]:
        """Process iterable of numbers and convert them to float."""
    ...
    def sum_non_negative_numbers(self, numbers: Iterable[Number]) -> float:
        """Sum non-negative numbers in the iterable."""
    ...
    def validate_numbers(self, numbers: Iterable[Number]) -> tuple[float, list[float]]:
        """Validate numbers by summing non-negative numbers and processing them."""
        try:
