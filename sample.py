...
def process_all_numbers(numbers: Iterable[Number]) -> Tuple[float, list[float]]:
    calculator = Calculator()
    return calculator.sum_non_negative_numbers(numbers), calculator.process_numbers(numbers)

def main() -> None:
    numbers = [1, 2, 3, 'a', 4.5, 6, 7, 8]
    result, data = process_all_numbers(numbers)
    print(result)
    print(data)
